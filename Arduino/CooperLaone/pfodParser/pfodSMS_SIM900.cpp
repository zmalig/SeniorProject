/**
TODO  
Alerts, close cmd, close connection (stop sending replys on closed connection)
 raw data (not sent if connection closed) but sent if timed out
...
pfodSMS_SIM900::pfodSMS_SIM900() {
  _setIdleTimeout(getDefaultTimeOut()); // default 10mins
 ...
 * just times out the current connection to allow new phone no to connect
 */
...
void pfodSMS_SIM900::startTimer() {
...
void pfodSMS_SIM900::setupSMS() {
  ...
  gprsPrint(F("AT+CIURC=1\r")); // set unsolicited result code enable
  pfodWaitForUtils::waitForOK(gprs, debugOut);
  gprsPrint(F("AT+CMGF=1\r")); // set text mode
  pfodWaitForUtils::waitForOK(gprs, debugOut);
  pfodWaitForUtils::dumpReply(gprs, debugOut);
  gprsPrint(F("AT+CSCS=\"GSM\"\r")); // set GSM chars set
  pfodWaitForUtils::waitForOK(gprs, debugOut);
  pfodWaitForUtils::dumpReply(gprs, debugOut);
  gprsPrint(F("AT+CSMP=17,167,0,0\r")); // set default text mode (not 8bit)
  pfodWaitForUtils::waitForOK(gprs, debugOut);
  pfodWaitForUtils::dumpReply(gprs, debugOut);
  // delete all on startup
#ifdef DEBUG_SETUP
  if (debugOut != NULL) {
    gprsPrint(F("AT+CMGL=\"ALL\"\r")); // print them all out first
...
void pfodSMS_SIM900::startSMS(const char* phoneNo) {
  ...
  gprsPrint(F("AT+CMGS=\"")); // send direct add " " back in
  gprsPrint(phoneNo);
  gprsPrint(F("\"\r"));
}
...
#endif // DEBUG
  gprsPrint(F("AT+CMGDA=\"DEL ALL\"\r")); // delete all msgs out and in on
  emptyIncomingMsgNo();
...
#endif // DEBUG
  gprsPrint(F("AT+CMGDA=\"DEL READ\"\r"));
}
