...
pfodSecurity::pfodSecurity(const char *_version) : parser(_version) {
  debugOut = NULL;
  pfod_Base_set = NULL;
  doFlush = false; // set to true for SMS /ESP-AT only, otherwise false
  initialization = true;
  idleTimeOut = 15 * 1000; // timeout use setIdleTimeOut(seconds); to change
  setIdleTimeoutCalled = false;
  _authorizationTimeout = 15 * 1000; // default 15sec
  closeConnection();
}
 ...
 */
void pfodSecurity::setIdleTimeout(unsigned long timeout) {
  setIdleTimeoutCalled = true;
  if (pfod_Base_set) {
    pfod_Base_set->_setIdleTimeout(timeout);
    idleTimeOut = 0;
    // ignore this and use pfod_Base idle timeout
#ifdef DEBUG_TIMER
    if (debugOut != NULL) {
      debugOut->println(F("setIdleTimeout() set idleTimeOut to "));
...
/*
 * authorization timeout in secs, default 15 sec
 */
void pfodSecurity::setAuthorizationTimeout(unsigned long timeout) {
  _authorizationTimeout = timeout * 1000;
}
...
void pfodSecurity::connect(pfod_Base* base_arg,  const __FlashStringHelper *hexKeyPgr, int eepromAddress) {
  ...
  if (setIdleTimeoutCalled) { // use the users setting
    setIdleTimeout(idleTimeOut / 1000); // once pfod_Base_set is not null setIdleTimeout passed through
  } else { // set the default
    setIdleTimeout(pfod_Base_set->getDefaultTimeOut()); // once pfod_Base_set is not null setIdleTimeout passed through
  }

  setAuthorizationTimeout(0); // disable authorization timeout idle timeout will let another connection through
...
void pfodSecurity::connect(Stream * io_arg, Print * raw_io_arg, const __FlashStringHelper * hexKeyPgr_arg, int eepromAddress_arg) {
  ...
  // if eepromAddress is <0 or hexKeyPtr NULL then no security
  ...
  eepromAddress = eepromAddress_arg;
  init();
...
#endif // DEBUG_PARSER
  parsing = true;
  connectionTimer = _authorizationTimeout;
#ifdef DEBUG_TIMER
  if (debugOut != NULL) {
 ...
 */
void pfodSecurity::setSecretKey(int eepromAddress, byte * key, int len) {
...
byte pfodSecurity::parse() {
  ...
  if ((!parsing) || (!mac.isValid())) {  // parsing and powerCycles > 0
    while (io->available()) {
      in = io->read();
  ...
  if ((cmd != DisconnectNow) && (cmd != 0)) {
    if (authorizing == AUTHORIZING_START) { // will not get this state if noPassword see setAuthorizeState()
      if ((cmd == AUTHORIZING_CMD) && (strlen((const char*) parser.getCmd()) == 1) && (parser.getArgsCount() == 0)) {
        ...
        connectionTimer = _authorizationTimeout; // reset timer
#ifdef DEBUG_TIMER
        if (debugOut != NULL) {
