...
pfodApp msg {.} --> {,<bg n><b><+3><w>~Uno Starter`1000~V7|A<bg bl>`0~D4 output is ~~Low\High~|B<bg bl>`0~D5 PWM Setting ~%`255`0~100~0~|!F<-6>~|!C<bg bl>`1~D6 Input is ~~Low\High~t|!E<-6>~|!D<bg bl>`775~A0 ~V`1023`0~5~0~|G<bg bl>~Plot A0}
 */
...
/* Code generated by pfodDesignerV2 V2.0.2001
...
pfodParser_codeGenerated parser("E8"); // create a parser to handle the pfod messages
...
void sendMainMenu() {
  ...
  parser.print(F("<bg n><b><+3><w>~Uno Starter`1000~V8"));
  parser.print(F("`1000"));
  // send menu items
...
void pfodParser_codeGenerated::init() {
  argsCount = 0; argsIdx = 0; args[0] = 0; args[1] = 0; cmdStart = args; versionStart = emptyVersion; parserState = 0xff; refresh = false;
}
...
byte pfodParser_codeGenerated::getParserState() {
  if ((parserState == 0xff) || (parserState == ((byte)'{')) || (parserState == 0) || (parserState == ((byte)'}')) ) {
    return parserState;
  } 
  return 0;
}
...
byte pfodParser_codeGenerated::parse(byte in) {
  if (in == 0xff) { return 0; }
  if ((parserState == 0xff) || (parserState == ((byte)'}'))) {
    parserState = 0xff;
    if (in == ((byte)'{')) { init(); parserState = ((byte)'{'); }
    return 0;
  }
  if ((argsIdx >= (0xff - 2)) && (in != ((byte)'}'))) { init(); return 0; }
  if (parserState == ((byte)'{')) {
    parserState = 0;
    if (in == ((byte)':')) {
