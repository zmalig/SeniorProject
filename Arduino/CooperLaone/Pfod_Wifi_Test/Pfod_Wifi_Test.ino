/* ===== pfod Command for Remote Light Switch ====
  pfodApp msg {.} --> {,<bg n><+4>~Porch Light`1000~V1|A<+7>`0~Light is ~~Off\On~}
*/
...
/* Code generated by pfodDesignerV2 V2.0.2170
...
// =============================== extra library and definitions for push button ===============================
...
#define pfodWifiWebConfigAP "FuckBitches"
...
bool wifiConnected = false; // if true connected to WiFi network and  pfod server started
...
unsigned long CONNECTION_TIMEOUT = 15000; // 15sec
...
void setup() {
  ...
  // turn light on
  cmd_A_var = 1;
  pinMode(cmd_A_pin, OUTPUT); // output for 'Light is' is initially HIGH,  Override RX setting from above
  digitalWrite(cmd_A_pin, cmd_A_var); // set output

  pinMode(gndPin, OUTPUT);
  digitalWrite(gndPin, 0); // set output LOW / GND   supplies gnd for Opto pin 5 which will gnd GPIO0 opto fires
    ...
    if (!client.connected()) {
      ...
      if (cmd != 0) { // have parsed a complete msg { to }
        ...
        if ('.' == cmd) {
          ...
          // in the main Menu of Remote Light Switch
...
void  processSwitch() {
  ...
  if (sw.isChanged()) { // debounced switch changed state Up or Down
    // toggle light i.e. turn off if on or on if off
    cmd_A_var = swap01(cmd_A_var);
    digitalWrite(cmd_A_pin, cmd_A_var); // set output
    // isChanged() is only true for one loop(), cleared when update() called again
    processSwitchChange(); // do other processing
  }
}

void processSwitchChange() {
  stopAP = true; // close access point if running
}
...
void handleRoot() {
  ...
  msg = "<html>"
        ...
        pfodWifiWebConfigAP
        "</h2>"
        "<p>Use this form to configure to set your Wifi network setting and the ip and port you want pfodApp to use when connecting.</p>"
  ...
  msg += " />"
         "<p class=\"password\">"
         "<label for=\"password\">Password for your WiFi network <br>(enter a space if there is no password, i.e. OPEN)</label><br>"
